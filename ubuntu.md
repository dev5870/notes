...
| `du -h /var/log/journal`                                                                                                                       | выводит размер указанной папки/файла                                   |
...
| `kill -9 111111`                                                                                                                               | убить процесс по pid                                                   |
| `sudo netstat -peanut \                                                                                                                        | grep ":80"`                                                            | выборка тех, кто слушает указанный порт |
| `sudo netstat -lntup`                                                                                                                          | выводит весь список портов, которые используются                       |
| `ssh -i ~/.ssh/id_rsa.pub -N -L 13306:127.0.0.1:3306 root@111.222.333.44`                                                                      | ssh тунель для подключения удаленной базы к локальному проекту         |
